...
└─────────────────────────────────────────────────────┘
 ...
 当前阶段单实例部署（Phase 2 增加引擎和 API 层水平扩展）。
 依赖方向由外向内：适配器 → 端口 → 领域层。
 领域层不依赖任何框架、数据库或 HTTP 协议。
```
...
| **插件引擎选择** | 翻译引擎服务端固定，插件端无法选择 | 允许用户在插件中选择 LLM / MTranServer / 自动 |
