...
NovelTrans 是一个全栈翻译平台，为网文作者和译者设计。它取代了传统的"复制粘贴到谷歌翻译"工作流，通过理解上下文、保持角色名一致性、并从历史翻译中学习 — 同时通过基于 RAG 的语义复用将 LLM API 成本降低 60-80%。
...
- **RAG 翻译记忆** — Redis HNSW 向量语义检索（OpenAI `text-embedding-3-small` 或 Ollama `bge-m3` 生成嵌入），相似度 ≥ 0.85 时跳过 LLM 调用，减少 60-80% API 成本
- **实体一致性管线** — 翻译前提取专有名词 → SHA-256 占位符替换（`__ENT_<hash>__`）→ 对照用户术语表翻译实体 → 译后还原，防止"John"在章节间变成"Jon"
- **多智能体协作 (AgentScope)** — 每章三个 AI 角色（译者 + 术语专家 + 润色师），按小说类型（玄幻/言情/科幻/悬疑/恐怖/日常）匹配不同 prompt
...
└──────────┘        └──────────┘    │  └───────────────┬────────────────────┘  │
        ...
        │ 8.0      │     │  Stack 7.4 │     │    │ FastAPI  │   │ 轻量 :8989  │
        │ Flyway   │     │  HNSW      │     │    │ :8000    │   │ 快速翻译    │
        └──────────┘     └────────────┘     │    └──────────┘   └────────────┘
        持久化            缓存 + 向量检索    │    多智能体团队     快速翻译
                                            │
                                 ┌──────────┴──────────┐
                                 │      Stripe         │
                                 │  结账 / 计费门户    │
...
| **Webhook** | `POST /stripe/webhook` | `POST /api/stripe/webhook` | Stripe 签名 |
...
| 文档 | 用途 |
|------|------|
| [`ARCHITECTURE.md`](ARCHITECTURE.md) | 系统架构、组件职责、数据流、缓存层级 |
| [`API_DOCUMENTATION.md`](API_DOCUMENTATION.md) | 完整 REST API 参考，含请求/响应示例 |
| [`SETUP.md`](SETUP.md) | 部署与本地开发指南 |
